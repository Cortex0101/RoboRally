...
public class RoboAI {
    ...
    public void updateAIPosition() {
        boardCopy.setCurrentPlayer(aiPlayerCopy);
        gameCopy.moveCurrentPlayerToSpace(boardCopy.getSpace(aiPlayerOrig.getSpace().x, aiPlayerOrig.getSpace().y));
    ...
    /**
     * Get command cards based on indexes corresponding to the command card in the programming deck at
    ...
    private List<CommandCard> getCommandCardsBasedOnIndexes(List<Integer> indexes) {
        ...
        for (int index : indexes) {
            CommandCard card = aiPlayerOrig.getCardField(index).getCard(); // We use the original player as this is the one that has the proper cards.
     ...
     * @return the programming cards selected from the programming deck, that gets the robot closest to the desired space
     */
